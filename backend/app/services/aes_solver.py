# app/services/aes_solver.py
import os

# ================= AES CONSTANTS =================
S_BOX = [

]

INV_S_BOX = [0]*256
for i,v in enumerate(S_BOX):
    INV_S_BOX[v] = i

RCON = [0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36]

# ================= HELPERS =================
def xtime(a): return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else (a << 1)
def gmul(a,b):
    r=0
    for _ in range(8):
        if b&1: r^=a
        a=xtime(a); b>>=1
    return r&0xFF

def add_round_key(s,k):
    for i in range(16): s[i]^=k[i]

def sub_bytes(s,inv=False):
    box = INV_S_BOX if inv else S_BOX
    for i in range(16): s[i]=box[s[i]]

def shift_rows(s, inv=False):
    for r in range(1, 4):
        row = [s[c*4 + r] for c in range(4)]
        if inv:
            row = row[-r:] + row[:-r]
        else:
            row = row[r:] + row[:r]
        for c in range(4):
            s[c*4 + r] = row[c]

def mix_columns(s, inv=False):
    for c in range(4):
        col = [s[c*4 + r] for r in range(4)]
        if inv:
            res = [
                gmul(col[0],14)^gmul(col[1],11)^gmul(col[2],13)^gmul(col[3],9),
                gmul(col[0],9)^gmul(col[1],14)^gmul(col[2],11)^gmul(col[3],13),
                gmul(col[0],13)^gmul(col[1],9)^gmul(col[2],14)^gmul(col[3],11),
                gmul(col[0],11)^gmul(col[1],13)^gmul(col[2],9)^gmul(col[3],14)
            ]
        else:
            res = [
                gmul(col[0],2)^gmul(col[1],3)^col[2]^col[3],
                col[0]^gmul(col[1],2)^gmul(col[2],3)^col[3],
                col[0]^col[1]^gmul(col[2],2)^gmul(col[3],3),
                gmul(col[0],3)^col[1]^col[2]^gmul(col[3],2)
            ]
        for r in range(4):
            s[c*4 + r] = res[r]

# ================= KEY EXPANSION =================
def key_expansion(key):
    Nk=len(key)//4; Nr=Nk+6
    w=list(key)
    def sub(w): return [S_BOX[b] for b in w]
    def rot(w): return w[1:]+w[:1]
    i=Nk
    while len(w)<16*(Nr+1):
        t=w[-4:]
        if i%Nk==0:
            t=sub(rot(t)); t[0]^=RCON[i//Nk]
        elif Nk>6 and i%Nk==4:
            t=sub(t)
        w += [w[-4*Nk+j]^t[j] for j in range(4)]
        i+=1
    return [w[i:i+16] for i in range(0,len(w),16)]

# ================= CORE =================
def aes_block(block,keys,enc=True):
    s=list(block); Nr=len(keys)-1
    add_round_key(s, keys[0] if enc else keys[Nr])
    for r in range(1,Nr):
        sub_bytes(s,not enc); shift_rows(s,not enc)
        mix_columns(s,not enc)
        add_round_key(s, keys[r] if enc else keys[Nr-r])
    sub_bytes(s,not enc); shift_rows(s,not enc)
    add_round_key(s, keys[Nr] if enc else keys[0])
    return bytes(s)

def pad(d): p=16-len(d)%16; return d+bytes([p])*p
def unpad(d): return d[:-d[-1]]

def encrypt(pt, key, mode, iv=None):
    if len(key) not in (16, 24, 32):
        raise ValueError("AES key must be 16 / 24 / 32 bytes")

    rk = key_expansion(key)
    data = pad(pt)
    out = b""

    if mode == "ECB":
        for i in range(0, len(data), 16):
            out += aes_block(data[i:i+16], rk, True)
        return out, None

    if mode == "CBC":
        iv = iv or os.urandom(16)
        prev = iv
        for i in range(0, len(data), 16):
            blk = bytes(a ^ b for a, b in zip(data[i:i+16], prev))
            enc = aes_block(blk, rk, True)
            out += enc
            prev = enc
        return out, iv

    rk=key_expansion(key); data=pad(pt); out=b""
    if mode=="ECB":
        for i in range(0,len(data),16):
            out+=aes_block(data[i:i+16],rk,True)
        return out,None
    if mode=="CBC":
        iv=iv or os.urandom(16); prev=iv
        for i in range(0,len(data),16):
            blk=bytes(a^b for a,b in zip(data[i:i+16],prev))
            enc=aes_block(blk,rk,True)
            out+=enc; prev=enc
        return out,iv

def decrypt(ct, key, mode, iv=None):
    if len(key) not in (16, 24, 32):
        raise ValueError("AES key must be 16 / 24 / 32 bytes")

    if len(ct) % 16 != 0:
        raise ValueError("Ciphertext length must be multiple of 16 bytes")

    rk = key_expansion(key)
    out = b""

    if mode == "ECB":
        for i in range(0, len(ct), 16):
            out += aes_block(ct[i:i+16], rk, False)
        return unpad(out)

    if mode == "CBC":
        if iv is None:
            raise ValueError("IV required for CBC")

        prev = iv
        for i in range(0, len(ct), 16):
            dec = aes_block(ct[i:i+16], rk, False)
            out += bytes(a ^ b for a, b in zip(dec, prev))
            prev = ct[i:i+16]
        return unpad(out)
